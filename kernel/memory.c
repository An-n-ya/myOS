...
static void* vaddr_get(enum pool_flags pf, uint32_t pg_count) {
    ...
    if (pf == PF_KERNEL) {
        ...
        bit_idx_start = bitmap_scan(&cur->userprog_vaddr.vaddr_bitmap, pg_count); // 分配内存
        if (bit_idx_start == -1) {
            return NULL;
        }
        while (count < pg_count) {
        ...
        // 0xc0000000 - PAGE_SIZE 作为用户3级栈，已经在start_process被分配了
...
void *get_a_page(enum pool_flags pf, uint32_t vaddr) {
    struct pool* mem_pool = pf & PF_KERNEL ? &kernel_pool : &user_pool; // 根据pf判断是用户内存池还是内核内存池
    // 保证原子操作
